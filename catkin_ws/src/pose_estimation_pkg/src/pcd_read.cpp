...
int main (int argc, char** argv)
{
  ...
  PointCloudRGBA::Ptr cloud_downsampled(new PointCloudRGBA);
  ...
  std::cerr << "PointCloud before downsampling: " << cloud_downsampled->width * cloud_downsampled->height 
       << " data points." << std::endl;
  ...
  */
  ...
  std::string file;
  file = "pcd_twoM200components.pcd";

  reader.read(file, *pcd);
  ROS_INFO("PCD pcd_twoM200components.pcd file loaded");
  ...
  pcl::fromPCLPointCloud2(*pcd_downsampled, *pcd_blob);
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); j < 1; ++it)
  {
    ...
    std::cout << "PointCloud representing the Cluster: " << pcd_clustered->size() << " data points." << std::endl;
    std::stringstream ss;
  ...
  file2save = "m200_clustered_downsampled.pcd";
  writer.write(file2save, pcd_saved);
  ROS_INFO("PCD m200_clustered.pcd file saved");
