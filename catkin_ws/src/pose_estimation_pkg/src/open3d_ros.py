...
def prepare_dataset(voxel_size):
    ...
    source = o3d.io.read_point_cloud("scaled_downsampled_m200.pcd")
    target = o3d.io.read_point_cloud("cloud_cluster_0.pcd")

    '''
    source.scale(0.001, center=source.get_center())
    target = o3d.io.read_point_cloud("pcd_twoM200components_3.pcd")

    points = np.asarray(target.points)
    target = target.select_by_index(np.where(points[:, 2] < 0.53)[0])
    target.scale(1.5, center=target.get_center())
    source.translate((0,0,0), relative=False)
    target.translate((0,0,0), relative=False)
    
    trans_init = np.eye(4)
    trans_init[:3, :3] = source.get_rotation_matrix_from_xyz((0, - np.pi / 2, np.pi / 2))
    trans_init[0, 3] = 200
    trans_init[1, 3] = - 60
